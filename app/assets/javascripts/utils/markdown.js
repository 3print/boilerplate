...
Markdown.dialects.Gruber = {
  block: {
    ...
    table: function table (block, next) {
      if ( !block.match( /^[\w\s]+\|/m ) )
        return undefined;

      block = String(block)

      var lines = block.split('\n').map(function(line){
        return line.split('|').filter(function(cell){
          return cell.replace(/^\s+|\s+$/g,'').length > 0;
        })
      });

      var hasHeader = lines.length > 1 && /^\s*-+\s*$/.test(lines[1][0]);
      var jsonlines = [];

      if(hasHeader) {
        var row = ['table_row'];
        lines.shift().forEach(function(cell){
          row.push(['table_head_cell', cell]);
        })
        jsonlines.push(['table_head', row]);
        lines.shift();
      }
...
Markdown.dialects.Gruber.inline = {
    ...
    "~~": function inlineStriketrough( text ) {
      var m = text.match( /(~~)(([\s\S]*?)\1)/ );

      if ( m && m[2] )
        return [ m[1].length + m[2].length, [ "strike", m[3] ] ];
      else {
        // TODO: No matching end code found - warn!
        return [ 2, "~~" ];
      }
    ...
    "%%%": function narrowWell( text ) {
      ...
      var consumed = 3 + res[ 0 ],
          children = res[ 1 ];

      return [ consumed, ["well", {class: 'well narrow'}].concat(children) ]
    },

    "%%": function well( text ) {
      var orig = String(text);
      // Inline content is possible inside wells
      var res = Markdown.DialectHelpers.inline_until_char.call( this, text.substr(2), '%%' );
      ...
      return [ consumed, ["well", {class: 'well'}].concat(children) ]
    },

    '^^^^': function lettrine3( text ) {
      var m = text.match( /(\^\^\^\^)(([\s\S]*?)\1)/ );

      if ( m && m[2] )
        return [ m[1].length + m[2].length, [ "lettrine", {class: 'lettrine level4'}, m[3] ] ];
      else {
        // TODO: No matching end code found - warn!
        return [ 4, "^^^^" ];
      }
    ...
    '^^^': function lettrine3( text ) {
      ...
      if ( m && m[2] )
        return [ m[1].length + m[2].length, [ "lettrine", {class: 'lettrine level3'}, m[3] ] ];
      else {
        // TODO: No matching end code found - warn!
        return [ 3, "^^^" ];
      }
    ...
    '^^': function lettrine3( text ) {
      ...
      if ( m && m[2] )
        return [ m[1].length + m[2].length, [ "lettrine", {class: 'lettrine level2'}, m[3] ] ];
      else {
        // TODO: No matching end code found - warn!
        return [ 2, "^^" ];
      }
    ...
    '^': function lettrine3( text ) {
      ...
      if ( m && m[2] )
        return [ m[1].length + m[2].length, [ "lettrine", {class: 'lettrine level1'}, m[3] ] ];
      else {
        // TODO: No matching end code found - warn!
        return [ 3, "^" ];
      }
...
Markdown.DialectHelpers.inline_until_char = function( text, want ) {
  ...
  while ( true ) {
    if ( text.slice(consumed, consumed + want.length) == want ) {
