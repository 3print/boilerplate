...
const checkboxGroupPredicate = all(inputPredicate('checkbox'), (i) => i.matches('.form-check-input-group input'));
const radioPredicate = inputPredicate('radio');
const filePredicate = inputPredicate('file');
const requiredPredicate = (i) => i.classList.contains('required') || i.hasAttribute('required');

const GROUP_SELECTOR = '.form-group';
...
export default {
  validators: [
    ...
    [
      all(checkboxGroupPredicate, requiredPredicate),
      (i18n, v, i) => {
        const inputs = asArray(document.querySelectorAll(`[name="${i.getAttribute('name')}"]`));
        return inputs.some(i => i.checked) ? null : i18n('unchecked_in_group')
      }
    ],
    [
  ...
  clean(input) {
    ...
    const group = parent(input, GROUP_SELECTOR);
    if(group) {
      group.classList.remove('is-valid');
      group.classList.remove('is-invalid');

      const prevError = group.querySelector(INVALID_FEEDBACK_SELECTOR);
