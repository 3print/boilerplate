...
import {asArray, always, parent, getNode, compose, detachNode, merge} from 'widjet-utils';
import 'widjet-validation';
import {validatePresence, nativeValidation} from 'widjet-validation/lib/validators';
import DEFAULT_VALIDATION_OPTIONS from '../js/widgets/validation';
...
const passwordPredicate = (i) =>
  i.type == 'password' && /password\]$/.test(i.name) && i.matches('.sign_up input')
const passwordConfirmationPredicate = (i) =>
  i.type == 'password' && /password_confirmation\]$/.test(i.name)
...
const DEFAULT_EVENTS = 'init load page:load turbolinks:load';
const VALIDATION_OPTIONS = {
  ...
  clean(input) {
    ...
    DEFAULT_VALIDATION_OPTIONS.clean(input);
  },
  onSuccess(input) {
    if(passwordPredicate(input)) {
      flagPasswordCriterium([]);
    }
    DEFAULT_VALIDATION_OPTIONS.onSuccess(input);
  },
  onError(input, res) {
    if(passwordPredicate(input)) {
      if(typeof res == 'string') {
        DEFAULT_VALIDATION_OPTIONS.onError(input, res);
