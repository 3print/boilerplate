module ModelsHelper
  ...
  def collection_counter(collection, controller=nil)
    return content_tag(:div, 0, class: 'badge bg-info tip-left float-end') if collection.size == 0

    model_class = collection.is_a?(Array) ? collection.first.try(:class) : collection.klass

    if controller.present?
      content_tag(:div, class: 'badge bg-info tip-left float-end', title: 'tips.models_count'.t(count: controller.resource_count, singular: "models.#{model_class.namespaced_name}".t.downcase, plural: "models.#{model_class.table_name}".t.downcase)) do
          concat(controller.resource_count_label)
      end
    else
      count = collection.size
      content_tag(:div, class: 'badge bg-info tip-left float-end', title: 'tips.models_count'.t(count: count, singular: "models.#{model_class.namespaced_name}".t.downcase, plural: "models.#{model_class.table_name}".t.downcase)) do
          concat(count)
      end
    end
  end
