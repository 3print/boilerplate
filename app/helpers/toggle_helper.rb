module ToggleHelper
  ...
  def build_toggle(method, resource, name, options={}, &block)
    ...
    if is_on
      ...
      if can?(off, resource)
        send method, controller.resource_path_for(resource, off), method: :put, class: 'btn btn-warning', title: title, data: {confirm: "confirmations.#{resource.class.table_name.singularize}.#{off}".t} do
          block.call(is_on)
        end
      end
    else
      ...
      if can?(name, resource)
        send method, controller.resource_path_for(resource,name), method: :put, class: 'btn btn-success', title: title, data: {confirm: "confirmations.#{resource.class.table_name.singularize}.#{name}".t} do
          block.call(is_on)
        end
      end
    end
