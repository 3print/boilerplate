class DateTimeInput < SimpleForm::Inputs::DateTimeInput
  def input(wrapper_options = nil)
    ...
    if type == :datetime
      ...
      input_html_options[:value] = object.send(attribute_name).to_formatted_s(:iso8601).split('T')[0]
      ...
      input_html_options[:value] = object.send(attribute_name).to_formatted_s(:time)
      s << @builder.text_field("#{attribute_name}__time", input_html_options)

      s << @builder.hidden_field("#{attribute_name}__offset", {
        value: object.send(attribute_name).to_formatted_s(:iso8601).split('+').last,
      })
